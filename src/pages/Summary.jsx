  ...
  useEffect(() => {
    async function load() {
      const [{ data: matches }, { data: goals }, { data: cards }, { data: matchPlayers }] = await Promise.all([
        ...
        supabase.from('goals').select('*, players(first_name, last_name)'),
      ...
      played.forEach(m => {
        ...
        if (m.score_us > m.score_them) wins++
        else if (m.score_us === m.score_them) draws++
        else losses++
      })
      ...
      ;(matchPlayers || []).filter(mp => mp.matches?.status === 'played').forEach(mp => {
        ...
        if (!stats[pid]) stats[pid] = { name, goals: 0, minutes: 0, yellow: 0, red: 0 }
      ...
      {loading ? (
        ...
        <>
          ...
          {teamStats && (
            <div style={{ marginBottom: 40 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                ...
                <TeamStat label="Bramki zdobyte" value={teamStats.goalsFor} color="var(--gold)" />
                <TeamStat label="Bramki stracone" value={teamStats.goalsAgainst} />
          ...
          <div className="card">
            <div style={{
              display: 'grid',
              gridTemplateColumns: tab === 'cards' ? '48px 1fr 80px 80px' : '48px 1fr 80px',
              ...
              {tab === 'cards' ? (
                ...
                <div style={{ fontFamily: 'var(--font-condensed)', fontSize: 11, letterSpacing: 2, color: 'var(--gold)', textTransform: 'uppercase', textAlign: 'center' }}>
                  {tab === 'goals' ? 'Gole' : 'Min'}
                </div>
              )}
            </div>
            ...
            {sorted[tab].length === 0 ? (
              <div style={{ padding: '24px 16px', color: 'var(--white-muted)', fontFamily: 'var(--font-condensed)' }}>
                Brak danych
              </div>
            ) : sorted[tab].map((p, i) => {
              const isTop = i === 0
              return (
                ...
                >
                  <div style={{
                    ...
                    color: isTop ? 'var(--gold)' : 'var(--white-muted)',
                  }}>
                    {i + 1}
                  </div>
